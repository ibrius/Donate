...
class Create extends CI_Model 
{
	...
	{		
		...
		$page_heading= mysql_real_escape_string($data['page_heading']);
				
		$query = "INSERT INTO users(page_id,message,email,currency,page_name,page_heading)
				VALUES ('$page_id','$message','$email','$currency','$page_name','$page_heading')";
	...
	{
		$query = $this->db->query("SELECT message, email, currency, page_name, page_heading FROM users WHERE page_id = '".$pid."'");
	...
	function updater($data,$where)
	{
		...
		$query = $this->db->query("UPDATE users SET message='$text', email='$email', currency='$currency', page_name='$page_name', page_heading='$page_heading' WHERE page_id='$where'");
